    ...
    <nav className="border-b border-slate-200 bg-white z-50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          {user ? (
            <>
              <span className="px-3 py-2 text-sm text-slate-600 truncate max-w-48">
                {user.email}
              </span>
              <form action={logoutUser}>
                <button
                  type="submit"
                  className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer"
                >
                  <LogOut size={16} />
                  Cerrar sesión
                </button>
      ...
      {menuOpen && (
        <div className="sm:hidden absolute top-full left-0 right-0 border-t border-slate-100 bg-white px-4 py-3 space-y-2 shadow-lg z-50">
          ...
          {user ? (
            <>
              <span className="block px-3 py-2 text-sm text-slate-500 truncate">
                {user.email}
              </span>
              <form action={logoutUser}>
                <button
                  type="submit"
                  className="flex items-center gap-1.5 w-full px-3 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer"
                >
                  <LogOut size={16} />
                  Cerrar sesión
                </button>
